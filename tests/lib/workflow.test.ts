...
async function createFakePiScript(dir: string): Promise<{ command: string; args: string[] }> {
	...
	const scriptContent = [
		...
		`let r;`,
		`if (sp.includes("initial artifacts") || sp.includes("bootstrap artifacts")) {`,
		...
		`       "## Applied methodology","Iterative.","## Main findings (by criterion)","[FACT] Findings.",`,
		`       "## Score evolution (consolidated scoreboard)","70/100.","## Firm decisions and active hypotheses","[DECISION] OK.",`,
		`       "## Identified risks and mitigations","[RISK] Complex.","## Final recommendations","Proceed.",`,
		`       "## Cross-references (artifacts by loop)","All."`,
		`  ].join("\\n");`,
...
export async function run(): Promise<void> {
	...
	await withTempDir(async (dir) => {
		...
		const scriptLines = [
			...
			'  tag("BACKLOG.md", "# BL\\n|ID|P|S|D|\\n|---|---|---|---|\\n|B1|P0|done|X|\\n|B2|P1|pend|Y|\\n|B3|P2|new|Z|")',
			'].join("\\n");',
			'const validReport = ["# Investigation Report","## Executive summary","Done.","## Context and investigation object","Analyzed.","## Applied methodology","Iterative.","## Main findings (by criterion)","[FACT] Findings.","## Score evolution (consolidated scoreboard)","70/100.","## Firm decisions and active hypotheses","[DECISION] OK.","## Identified risks and mitigations","[RISK] Complex.","## Final recommendations","Proceed.","## Cross-references (artifacts by loop)","All."].join("\\n");',
